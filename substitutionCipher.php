...
$initialLetters = readline();
$encryptingLetters = readline();
$stringToEncrypt = readline();
$stringToDecrypt = readline();

$cipherKey = createEncryptingDictionary($initialLetters, $encryptingLetters);
$encryptedString = encryptString($cipherKey, $stringToEncrypt);
$decryptedString = decryptString($cipherKey, $stringToDecrypt);

function createEncryptingDictionary($initialLetters, $encryptingLetters)
{
    $keysArray = str_split($initialLetters);
    $valuesArray = str_split($encryptingLetters);
...
{
    $encryptedString = '';
    foreach (str_split($stringToEncrypt) as $character){
        if (in_array($character, array_keys($cipherKey))){
...
{
    $decryptedString = '';
    foreach (str_split($stringToDecrypt) as $item){
        if (in_array($item, array_values($cipherKey))){
            foreach ($cipherKey as $key => $value){
                if($item === $value){
                    $decryptedString .= $key;
                }
            }
